...
unsigned char* MAC = new unsigned char[32];


bool bool_check_integrity();

//get_mac will get the mac of an encrypted file
unsigned char* get_mac(const unsigned char *pt, int len)
{
	...
	const EVP_MD *toRun = EVP_sha512();
    	...
    	digest = HMAC(toRun, key, strlen((const char *)key), pt, len, NULL, NULL);    
...
int main(int argc, char * argv[])
{
	...
	if((ifstream("passwd_file")) && (ifstream("master_passwd"))) //If both passwd_file and master_passwd exist
	{
		...
		for(int i = 0, j = i - 256; i < 336; i++, j++)
		{
			if(i < 256)
				saltedpass[i] = salt[i];
			else
				saltedpass[i] = master_passwd[j];
		}
	...
	else
	{
		...
		fwrite(&salt, sizeof(char), 256, wrt);
